Seurat_wrap = function(
  ...
  SCE_list[[1]] <- SCE_list[[1]][rownames(SCE_list[[2]]),]
  nPCs=20
  if(nrow(SCE_list[[2]])==25)nPCs=10
  ...
  # })
  seurat.list = SCE_list
  
  if(method=="rlsi"){
    seurat.list <- lapply(seurat.list, function(sc){RunUMAP(pbmc, reduction = 'lsi', dims = 1:30, reduction.name = "umap.atac", reduction.key = "atacUMAP_")})
    seurat.list <- lapply(seurat.list, function(sc)RunSLSI(object = sc))
  }
  ...
  #   x <- RunPCA(x, features = features, verbose = FALSE, npcs=20)
  # })
  
  seurat.anchors <- FindIntegrationAnchors(object.list = seurat.list, reference = c(1), reduction = method, dims = 1:20)
  seurat.combined <- IntegrateData(anchorset = seurat.anchors, dims = 1:nPCs, k.weight = 20)
  
  seurat.combined <- ScaleData(seurat.combined, verbose = FALSE)
  seurat.combined <- RunPCA(seurat.combined, verbose = FALSE, npcs = nPCs)
...
UINMF_wrap = function(
  ...
  data_liger <- selectGenes(data_liger, unshared = unshared,
                            useUnsharedDatasets = which(has_unshared),
                            unsharedThresh = 0.4)
  
  # rescale the data
  data_liger <- scaleNotCenter(data_liger)
  # set the number of dimensions to be bounded by the number of scaled data
  ncomponentsSubset <- min(ncomponentsSubset, ncol(rliger::scaleData(data_liger)[[1]]))
  ...
  # library(RcppPlanc)
  # data_liger <- runIntegration(data_liger, k=ncomponentsSubset, method="UINMF")#, use.unshared = unshared)
  print("RUNNING Integration ")
  data_liger <- runIntegration(data_liger, k=ncomponentsSubset, method="onlineINMF")#, use.unshared = unshared)
  #  rliger::run
...
MultiMAP_wrap = function(assay_list, verbose = FALSE) {
  ...
  reticulate::py_run_string("import MultiMAP")
  ...
  py_run_string("adata = MultiMAP.Integration([ad_1, ad_2], ['X_pca', 'X_pca'])")
